...
// stage 1. Короткая запись (не требует прописывать return)
const arrowFunc = (x = 0, y = 0) => x + y;
console.log(arrowFunc(2, 22));

// stage 2. Если нет аргументов
const arrowWithoutArg = () => console.log("Hello World!");
arrowWithoutArg();

// 3. Если stage 1 аргумент (можно без скобок)
const arrowOneArg = (x) => (x * 100);
console.log(Math.round(arrowOneArg(1.11)));

// 4. Если нужно писать больше, чем stage 1 строку
...
// stage 1. У обычной функции есть псефдоколлекция arguments, хранящий все аргументы, которые хранит функция
...
// stage 2. Стрелочные функции не имеют собственного контекста, поэтому this ссылается на уровень выше
const obj = {
    firstname: "Oleg",
    lastName: "Ivanov",
    age: 23,
