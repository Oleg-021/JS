...
arr.filter(inArray([stage 1,stage 2,3])) – выбирает только элементы, совпадающие с одним из элементов массива
arr.filter(notInArray([stage 1,stage 2,3])) – выбирает только те элементы,
которые не совпадают ни с одним из элементов массива
Пример:

let arr = [stage 1, stage 2, 3, 4, 5, 6, 7, true, undefined, NaN];

console.log(arr.filter(inRange(3, 6))); // [3, 4, 5, 6]
console.log(arr.filter(inArray([stage 1, stage 2, 10, undefined]))); // [stage 1, stage 2, undefined]
console.log(arr.filter(notInArray([stage 1, stage 2, 3, 4, 5, 6, 7, true]))); // [undefined, NaN] */
